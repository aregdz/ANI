...
from .forms import EmailLoginForm, EmailRegisterForm, StoryForm
...
def register_view(request):
    ...
    return render(request, 'stories/auth.html', {
        'form': form,
        'title': 'Регистрация',
        'hint': 'Введите email и придумайте пароль. После регистрации подтвердите email по ссылке из письма.',
    })
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not user.email_verified and not user.is_admin_owner:
                messages.error(request, 'Сначала подтвердите email. Проверьте почту.')
                return redirect('login')
...
def story_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                'История опубликована' if request.user.is_admin_owner else 'История отправлена на подтверждение'
            )
...
def admin_panel(request):
    ...
    users = User.objects.all().order_by('-is_admin_owner', 'email')
